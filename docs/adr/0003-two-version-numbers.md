...
`tauri.conf.json` and `Cargo.toml` at tag time, so `main` always states the last released
version. The plugin API version (`PLUGIN_API_VERSION`, mirrored by `WIDGET_API_VERSION` and
