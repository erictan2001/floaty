 ...
 *
 * The floatie it drags is put back where it was found.
 *
 * Usage: node verify/drag.mjs [--port 9222] [--id file-41]
...
const wantId = flag("id", null);

const sessions = [];
const skip = async (message) => {
  console.error(`drag: ${message}`);
  for (const s of sessions) await s.close().catch(() => undefined);
  process.exit(2);
};
const failed = async (message, detail = []) => {
  console.error(`drag: FAILED — ${message}`);
...
for (let nth = 0; nth < overlays.length; nth += 1) {
  ...
  const survey = await session.evaluate(SURVEY);
  surveyed.push({ nth, session, ...survey });
}
const source = wantId
  ? surveyed.find((s) => s.ids.includes(wantId))
  : surveyed.reduce((best, s) => (s.ids.length > (best?.ids.length ?? 0) ? s : best), null);
if (!source) {
  await skip(
    wantId ? `no overlay window has ${wantId}` : "no floatie lives on any screen to drag",
  );
}
const other = surveyed.find((s) => s.screen !== source.screen);
if (!other) await skip(`both overlays report the same screen (${source.screen})`);

const id = wantId ?? source.first;
...
// Put it back: the probe moves a real floatie, and the way home is the same handover.
...
console.log(
  `drag: ${id} crossed ${source.screen} → ${other.screen} → ${source.screen} ` +
